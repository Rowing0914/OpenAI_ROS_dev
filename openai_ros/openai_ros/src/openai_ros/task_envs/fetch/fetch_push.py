...
class FetchPushEnv(fetch_env.FetchEnv, utils.EzPickle):
    ...
    def get_params(self):
        ...
        self.goal = np.array([self.goal_ee_pos["x"], self.goal_ee_pos["y"], self.goal_ee_pos["z"]])
    ...
    def _get_obs(self):
        ...
        grip_pos = np.array([grip_pos_v.pose.position.x, grip_pos_v.pose.position.y, grip_pos_v.pose.position.z])
        dt = self.get_elapsed_time()
        grip_velp = (grip_pos - self.prev_grip_pos)/dt       # Velocity(position) = Distance/Time
        object_data = self.obj_positions.get_states()        # the pose and rotation of the cube/box on a table
        object_pos = object_data[:3]                         # pose of cube
        object_rot = object_data[3:]                         # rotation of cube
        object_velp = (object_pos - self.prev_object_pos)/dt # Velocity(position) = Distance/Time
        object_velr = (object_rot - self.prev_object_rot)/dt # Velocity(rotation) = Rotation/Time
    ...
    def calculate_if_done(self, movement_result, goal, current_pos):
        ...
        if movement_result:
            ...
            position_similar = np.all(np.isclose(goal, current_pos, atol=1e-02))
